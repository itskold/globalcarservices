                ...
                <Card key={category} className="hover:shadow-xl transition-all duration-300 group-hover:scale-[1.02]">
                  <CardContent className="pt-6">
                    <div className="text-center">
                      ...
                      <h3 className="text-xl font-semibold mb-2">{t(`categories.types.${category.toLowerCase()}`)}</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-600">{t(`specs.fuel.${car.fuel.toLowerCase()}`)}</span>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    <ul className="space-y-2">
                      {car.features.slice(0, 3).map((feature, idx) => (
                        <li key={idx} className="flex items-center text-sm text-gray-600">
                          ...
                          {t(`features.${feature.toLowerCase().replace(/\s+/g, "_")}`)}
                        </li>
                      ))}
                    </ul>
                    <Button
                    ...
                    >
                      <Link href={`/cars/vehicle/${car.id}`}>{t("vehicle.view_details")}</Link>
                    </Button>
                  </div>
                </CardContent>
              </Card>
      ...
      <section className="py-16 bg-[#050b20] text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              <Link href="/appointment">{t("cta.appointment")}</Link>
            </Button>
          </div>
        </div>
      </section>
