...
export default function MinibusPage() {
  ...
  useEffect(() => {
    const loadVehicles = async () => {
      try {
        ...
        // Filtrer uniquement les minibus
        const minibuses = allVehicles.filter(vehicle => vehicle.category === "Minibus")
        setVehicles(minibuses)
      } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">
              <EditableTranslationText namespace="rental.minibus" id="hero.title" />
            </h1>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              <EditableTranslationText namespace="rental.minibus" id="hero.subtitle" />
            </p>
          </div>
        </div>
      </section>
      ...
      {/* Filters */}
      <RentalFilters onFiltersChange={setFilters} vehicleType="minibus" />

      {/* Results Count */}
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-gray-600">
            <EditableTranslationText namespace="rental.minibus" id="results.showing" />{" "}
            {filteredVehicles.length}{" "}
            <EditableTranslationText namespace="rental.minibus" id="results.results" />
          </p>
        </div>
      </section>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="text-2xl font-bold text-[#050b20]">
                      <EditableTranslationText namespace="rental.minibus" id="fromPrice" /> €{vehicle.pricing[0].price}
                      <span className="text-sm font-normal text-gray-600">
                        <EditableTranslationText namespace="rental.minibus" id="pricePerHour" />
                      </span>
                    </div>
                    ...
                    >
                      <Link href={`/${locale}/rental/vehicle/${vehicle.id}`}>
                        <EditableTranslationText namespace="rental.minibus" id="viewDetails" />
                      </Link>
                    </Button>
                  </div>
                </CardContent>
