...
export default function MinibusPage() {
  ...
  const minibuses = vehicles.filter(vehicle => vehicle.category === "Minibus")

  // Filtrer les véhicules basés sur les filtres
  const filteredVehicles = minibuses.filter((vehicle) => {
      ...
      <section className="bg-gradient-to-br from-[#050b20] to-[#0a1530] text-white py-20 rounded-t-[4rem]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">Minibussen</h1>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              Comfortabel groepsvervoer voor maximaal 9 personen
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-gray-600">
            {filteredVehicles.length} van {minibuses.length} minibussen gevonden
          </p>
        </div>
      </section>
