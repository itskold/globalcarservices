...
export default function RefrigeratedVanPage() {
  ...
  // Filtrer uniquement les koelwagens
  const refrigeratedVans = vehicles.filter(vehicle => vehicle.category === "Koelwagen")

  // Filtrer les véhicules basés sur les filtres
  const filteredVehicles = refrigeratedVans.filter((vehicle) => {
      ...
      <section className="bg-gradient-to-br from-[#050b20] to-[#0a1530] text-white py-20 rounded-t-[4rem]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">Koelwagens</h1>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              Professionele koelwagens voor het transport van gekoelde en bevroren goederen
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-gray-600">
            {filteredVehicles.length} van {refrigeratedVans.length} koelwagens gevonden
          </p>
        </div>
      </section>
